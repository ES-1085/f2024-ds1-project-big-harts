...
trap_data <- true_lobsters %>%
  ...
  arrange(desc(n))  # Sort by the count of traps in descending order
...
```{r tag-ID}
homeport_order <- true_lobsters %>%
  count(`HOME PORT`) %>%
  arrange(desc(n)) %>%
  pull(`HOME PORT`)

# Reorder `HOME PORT` based on total counts
true_lobsters$`HOME PORT` <- factor(true_lobsters$`HOME PORT`, levels = homeport_order)

# Create the bar chart
ggplot(data = true_lobsters, mapping = aes(x = `TAG ID NUMBER`, fill = `HOME PORT`)) +
  geom_bar() +
  labs(x = "Tag ID Number", y = "Number of Traps",
...
homeports_known$`HOME PORT` <- factor(homeports_known$`HOME PORT`, 
   levels = homeports_known %>%
   arrange(desc(trap_count)) %>%
   pull(`HOME PORT`))

ggplot(data = homeports_known, aes(x = "", y = trap_count, fill = `HOME PORT`)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar(theta = "y") +
  labs(
    fill = "Home Port",
    title = "Distribution of Trap Counts by Home Port on Hart Island"
  ) +
  theme_void() +
