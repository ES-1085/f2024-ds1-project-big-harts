...
```{r tag-ID-&-port}
# Filter out unwanted entries (N/A, empty strings) from `HOME PORT`
true_lobsters_filtered <- true_lobsters %>%
  filter(!is.na(`HOME PORT`) & `HOME PORT` != "N/A" & `HOME PORT` != "")

# Aggregate data: Count the number of traps per `HOME PORT`
homeport_trap_count <- true_lobsters_filtered %>%
  group_by(`HOME PORT`) %>%
  summarize(total_traps = n()) %>%
  arrange(desc(total_traps))

# Plot: Single bar graph of traps by `HOME PORT`
ggplot(data = homeport_trap_count, aes(x = reorder(`HOME PORT`, -total_traps), y = total_traps, fill = `HOME PORT`)) +
  geom_bar(stat = "identity") +
  labs(x = "Home Port", y = "Total Number of Traps",
       title = "Total Number of Traps Found on Hart Island by Home Port") +
  scale_fill_viridis_d(option = "D", name = "Home Port") +
  theme_linedraw() +
<<<<<<< HEAD
...
# Step 4: Calculate the weight for each line based on trap count
homeports_known <- homeports_known |>
  mutate(weight = trap_count / max(trap_count) * 10)
...
leaflet() |>
  ...
  addPolylines(
    data = lines,
    weight = homeports_known$weight, # Use precomputed weight
    color = "blue",
    opacity = 20,
  ...
  # Add markers for unknown homeports if any
  addCircleMarkers(
    data = lobsters_anon |> filter(is.na(`HOME PORT`)),
    lng = hart_island["longitude"],
    lat = hart_island["latitude"],
    color = "gray",
    radius = 3,
    label = "Unknown Homeport"
  ) %>% 
